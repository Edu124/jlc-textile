...
export default function FinishedGoods() {
  ...
  const columns = [
    ...
    { header: "Unit", key: "unit" }, { header: "In Stock", cell: (r) => num(r.quantity, 2) },
  ...
  return (
    <div>
      <PageHeader title="Finished Goods" />
      <div className="mb-4 grid grid-cols-3 gap-4">
        <StatCard label="Products" value={rows?.length || 0} icon="◼" />
...
function Adjust({ row, onClose, onSaved }) {
  const [qty, setQty] = useState(String(row.quantity)); const [reason, setReason] = useState("Manual Adjustment");
  const [busy, setBusy] = useState(false); const [err, setErr] = useState("");
  const save = async () => {
    setBusy(true); setErr("");
    try { await api.post("/api/finished-goods/adjust", { product_id: row.id, new_quantity: Number(qty) || 0, reason }); onSaved(); }
